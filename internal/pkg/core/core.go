...
// see https://patorjk.com/software/taag/#p=display&f=Banner3-D&t=web_demo
const _UI = `
'##:::::'##:'########:'########:::::::::::'########::'########:'##::::'##::'#######::
 ##:'##: ##: ##.....:: ##.... ##:::::::::: ##.... ##: ##.....:: ###::'###:'##.... ##:
 ##: ##: ##: ##::::::: ##:::: ##:::::::::: ##:::: ##: ##::::::: ####'####: ##:::: ##:
...
func New(logger *zap.Logger, options ...Option) (Mux, error) {
	...
	if env.Active().IsDev() {
		fmt.Println(fmt.Sprintf("Swagger:http://%s/swagger/index.html", configs.ProjectDomain+configs.ProjectPort))
	}
	...
	withoutTracePaths := map[string]bool{
		...
		"/api/authorization/query_certification": true,
	}
	...
	mux.engine.Use(func(ctx *gin.Context) {
		...
		defer func() {
			...
			body := "tooBig"
			if !strings.Contains(traceHeader["Content-Type"], "multipart/form-data") {
	...
	system := mux.Group("/system")
	{
		...
		system.GET("/health", func(ctx Context) {
			resp := &struct {
				...
				Host:        ctx.Host(),
				Status:      "ok",
				Version:     configs.ProjectVersion,
			}
			ctx.Payload(resp)
		})
	}
